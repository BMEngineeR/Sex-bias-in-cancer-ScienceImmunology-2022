...
data_dir <- "data/MB49_JS_day16/"
fig_dir <- "figures/MB49_JS_day16/"

group_colors <- c("#0067ff","#00b145","#ff00ff")

# read gated data from Johanna
flow_df <- fread(paste0(data_dir,"47_files_concat-170656400341635_29.csv")) %>%
  separate(data = .,
           col = "OmiqFileIndex",
           sep = "_",
           into = c("day","ID"),
           remove = FALSE) %>%
  mutate(day = substr(day,8,9)) %>%
  mutate(group = ifelse(substr(ID,1,1) == "C","Cast M",
                        ifelse(substr(ID,1,1) == "F", "Sham F","Sham M")),
         group = factor(group, levels = c("Sham M","Cast M","Sham F"))) %>%
  filter(day == "16")
save(flow_df,
     file = paste0(data_dir,"flow_df_day16.RData"))
load(paste0(data_dir,"flow_df_day16.RData"))

# use all markers for JS's data
markers <- c("KI67",
             "CD27",
             "GITR",
             "CD44",
...
# save(umap_df, file = paste0(data_dir,"umap_df_day16.RData"))
load(paste0(data_dir,"umap_df_day16.RData"))
...
save(PCA_gated,
     file = paste0(data_dir,"PCA_gated_day16.RData"))
load(paste0(data_dir,"PCA_gated_day16.RData"))
...
PCA_means <- PCA_df %>% 
  ...
  summarize(PC1 = mean(PC1),
            PC2 = mean(PC2)) %>%
  ungroup() %>%
  mutate(sample = substr(sample,5,12))
...
heatmap_df <- flow_df %>%
  select(-c(OmiqFileIndex,day,ID,group,UM1,UM2))
...
# Umap cluster labels
########################################################################################################
# umap_df$cluster <- as.factor(cyt@meta.data$cluster.id)
...
save(scores,
     file = paste0(data_dir,"scores_ucell_day16.RData"))
load(paste0(data_dir,"scores_ucell_day16.RData"))
...
adjusted_PE_count_plot_all <- ggplot(PE_count_sub,aes(x = group,y = adj_count,fill = group)) +
  ...
  annotate(geom = "text",x = 3, y = PE_count_sub$adj_count[3] - 100,label = paste0(abs(round((PE_count_sub$adj_count[3]/PE_count_sub$exp_count[3])*100,0)),"% vs. expected"))
...
naive_sample_plots <- ggplot(naive_sample_counts,
  ...
  labs(subtitle = paste0("ANOVA p-value = ",round(lmp(lm(data = naive_sample_counts, formula = prop_naive ~ group)),4)))
...
PE_sample_plots <- ggplot(PE_sample_counts,
  ...
  xlab(NULL)+
  ylab(NULL)+
  labs(subtitle = paste0("ANOVA p-value = ",round(lmp(lm(data = PE_sample_counts, formula = prop_PE ~ group)),4)))

TukeyHSD(aov(data = PE_sample_counts, formula = prop_PE ~ group))
write.csv(PE_sample_counts,
          file = paste0(fig_dir,"PE_sample_counts.csv"),
          row.names = FALSE,
          quote = FALSE)
...
act_sample_plots <- ggplot(act_sample_counts,
  ...
  xlab(NULL)+
  ylab(NULL) +
  labs(subtitle = paste0("ANOVA p-value = ",round(lmp(lm(data = act_sample_counts, formula = prop_act ~ group)),4)))
